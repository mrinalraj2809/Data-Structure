/***************************************************************************
* Program 	:	06CircularQueue.c
* Description :	Implementation of Circular Queue to perform insertion,deletion and display
* Author		:Mrinal Raj
...
int fnDeleteFront(queue_t *q)
{
	...
	if(q->iRear==q->iFront)
	{
		...
		q->iFront=(q->iFront+1)%QUEUE_SIZE;
	return element;
}
/***************************************************************************
...
{
	return ((q.iRear+1)%QUEUE_SIZE==q.iFront);
}
/***************************************************************************
* Function 			:	fnDisplay
...
void fnDisplay(queue_t q)
{
	...
	{
		for(i=q.iFront;i<=QUEUE_SIZE-1;i++)
			printf("%d\t",q.Queue[i]);
		for(i=0;i<=q.iRear;i++)
			printf("%d\t",q.Queue[i]);
		printf("\n");
